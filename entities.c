...
static const char *entities_strchr(const char *s, int c, size_t len)
{
    const char  *p = s;
...
static bool parse_entity(
    const char *current, char **to, const char **from, size_t *remaining)
{
    size_t     len;
    const char *end;
    ...
    end = entities_strchr(current, ';', len + 1);
    if(!end) {
        return 0;
    }
...
size_t entities_decode_html(char *dst, const char *src, size_t src_len)
{
    ...
    while ((current = entities_strchr(from, '&', remaining))) {
        memmove(to, from, (size_t)(current - from));
        to += current - from;
        remaining = remaining - (size_t)(current - from) - 1;

        if(parse_entity(current, &to, &from, &remaining)) {
            continue;
        }
